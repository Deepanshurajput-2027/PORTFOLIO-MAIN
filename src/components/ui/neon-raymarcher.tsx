...
vec3 getHolographicMaterial(vec3 normal, vec3 viewDir, float time) {
    ...
    // Custom Yellow/Gold iridescence for brand match
    vec3 goldShades = vec3(
        0.9 + sin(hue) * 0.1,  // High Red -> Yellow
        0.7 + sin(hue + 1.0) * 0.2, // Medium Green -> Yellow/Gold
        0.1 + sin(hue + 2.0) * 0.1 // Low Blue
    );
    
    return goldShades * fresnel * 1.5;
}
...
void main() {
    ...
    if(totalDist < tMax) {
        ...
        vec3 rimColor = vec3(1.0, 0.8, 0.2) * rimLight * 0.5; // Golden edge
        
        // Ambient occlusion simulation
        float ao = 1.0 - smoothstep(0.0, 0.3, totalDist / tMax);
        
        vec3 baseColor = vec3(0.15, 0.1, 0.05); // Dark gold base
        color = baseColor * (0.1 + diff * 0.4) * ao;
        color += iridescent * (0.8 + diff * 0.2);
        color += vec3(1.0, 0.9, 0.6) * spec * 0.6;
        color += rimColor;
...
const ScreenPlane: FC<ScreenPlaneProps> = ({ animationState, amount }) => {
  ...
  return (
    <Plane args={[2, 2]} scale={[viewport.width / 2, viewport.height / 2, 1]}>
      <shaderMaterial
        ref={materialRef}
        uniforms={uniforms}
...
export const NeonRaymarcher: FC = () => {
  ...
  // Re-create animation state when amount changes to ensure arrays are correct size
  const animationState = useMemo(() => createInitialState(amount), [amount])
  ...
  return (
    <div className='w-full h-full'>
      <Canvas
        key={amount} // Force remount on resize to reset WebGL state for new shader
        camera={cameraConfig}
        dpr={1}
        frameloop="always"
        gl={{
